...
<body>
    ...
    <a href="todo.php"><h2 class="h22">Tasks(Touts)</h2></a>
                       <h2 class="h22">Tasks(En cours)</h2>
            <hr>
		<section class="task-list">

            <?php 

            $R = $conect->query("SELECT idtache, todo, cas FROM tache WHERE iduser = '$id' AND (cas IS NULL OR cas='')  "); //pour affiche les tache d'utilisateur
            while($T = $R->fetch_assoc()){
                echo '<td>'. $T['todo'].' </td>';
                  
                    echo '<a href="fait.php?Ctacheid='.$T['idtache'].'"><button>Fait!</button></a>';
